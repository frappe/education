...
7. **Phiếu thu học phí (hóa đơn tháng)** (đã chốt và làm 2026-09-19): mỗi học sinh một phiếu/tháng (múi giờ của giáo viên). Nút "Create receipts" tạo bản nháp cho mọi học sinh có buổi `attended` trong tháng: mỗi khoá một dòng = số buổi × giá (giá riêng của học sinh nếu có, không thì giá khoá; buổi giá 0 và buổi đã huỷ không tính, `absent` không tính). Giáo viên sửa được dòng, thêm dòng tay (kể cả số âm để giảm giá), ghi chú, hạn thanh toán; "Fill from attendance" tính lại từ điểm danh và giữ dòng thêm tay. Gửi phiếu = học sinh thấy trong portal + email (dev outbox trên staging), cấp số `INV-YYYYMM-0001` đếm tăng trong từng tháng ngay trong câu lệnh gửi (không trùng, không thủng), chụp tên và thông tin thanh toán của giáo viên (tên người nhận, SĐT, ngân hàng, số tài khoản, chủ tài khoản, lời nhắn; đều tuỳ chọn, không có mã QR). Từ lúc gửi phiếu không sửa được (DB có trigger chặn cả sửa, đổi số, xoá). Sau đó: đánh dấu đã trả (và trả lại "chưa trả" nếu bấm nhầm), hoặc huỷ kèm lý do (chỉ giáo viên thấy lý do; số cũ không dùng lại; tháng đó tạo được phiếu mới). Nếu điểm danh đổi sau khi gửi, phiếu vẫn giữ nguyên và giáo viên thấy cảnh báo. In/lưu PDF bằng trình duyệt (chỉ in phần phiếu). Ghi chú "không phải hóa đơn thuế" ở chân phiếu. Excel và email thật để sau (chờ chọn SMTP).
