...
import { useUnreadPolling } from "@/features/notifications/useNotifications";
...
<template>
  <AppShell v-if="session.me" :nav-label="t.label" :menu-label="t.menu">
    ...
    <template #topbar><NotificationBell /></template>
    <template #nav>
      ...
      <template v-if="session.isTeacher">
        ...
        <AppNavItem to="/invoices" icon="invoice">{{ t.invoices }}</AppNavItem>
      </template>
