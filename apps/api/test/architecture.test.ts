...
describe("data access rules", () => {
  ...
  it("never builds SQL by joining strings (only a few fixed, named pieces are allowed)", () => {
    // `${COLUMNS}`, `${where}`, `${ACTIVE_SEATS}`, `${TARGETED}`, `${VISIBLE}`, `${FROM}`, `${MINE}`, `${BILLED_PICK}`, `${BILLED_ANY}`, `${TAKEN_ON_INSERT}`, `${TAKEN_ON_UPDATE}`, `${EMAIL_STATE}`, `${NEW_ID}`, `${COURSE_FROM}`, `${COURSE_COLUMNS}` and `${onTime}` are constants written in the repos. `${placeholders(n)}` only
    // makes "?,?,?". No text from a request can reach these.
    const SAFE =
      /^(COLUMNS|where|ACTIVE_SEATS|TARGETED|VISIBLE|FROM|MINE|BILLED_PICK|BILLED_ANY|TAKEN_ON_INSERT|TAKEN_ON_UPDATE|EMAIL_STATE|NEW_ID|COURSE_FROM|COURSE_COLUMNS|onTime|placeholders\((chunk|ids)\.length\))$/;
