...
def spread_across_days(count, num_days=5):
	...
	step = num_days / count
	return sorted(set(min(int(i * step + step / 2), num_days - 1) for i in range(count)))
...
def load_configuration(stream_filter=None):
	...
	try:
		...
		subject_rules = [s for s in all_subject_rules if s["subject"] in taught_subjects]
		...
		if not teacher_preferences:
			...
			frappe.throw(f"No teacher preferences found{scope_hint}. Check the Teachers tab.")
		...
		frappe.log_error(f"Failed to load configuration: {str(e)}", "Timetable Generator")
		raise
...
def get_period_slots(timetable_doc):
	...
	if not time_slots:
		frappe.throw("No time slots defined. Please add time slots in the Slot & Breaks tab.")
...
def _stream_load_on_day(day_str, stream, subject_stream_daily):
	...
	return sum(
		v for (d, _, sg), v in subject_stream_daily.items() if d == day_str and sg == stream
	)
...
def balanced_pass(
	...
	for item in list(remaining_items):
		subject, stream = item["subject"], item["stream"]
		sorted_day_indices = _sorted_days_for_item(item, school_days, subject_stream_daily)
...
def _pre_populate_from_db(
	...
	time_to_idx = {
		(slot["from_time"], slot["to_time"]): idx for idx, slot in enumerate(period_slots)
	}
	...
	try:
		...
		for ap in assessment_plans:
			...
			for period_idx, slot in enumerate(period_slots):
				...
				if slot_start and slot_end and slot_start < ap_end and slot_end > ap_start:
...
def diagnose_unscheduled(unscheduled_items, config, scheduled_items):
	...
	for (subject, stream), missed in groups.items():
		...
		capable = [
			t for t in teacher_prefs if t["subject"] == subject and t["stream"] == stream
		]
		if not capable:
			reasons.append(f"No teacher is assigned to '{subject}' for stream '{stream}'.")
			hints.append(
...
def create_result_document(
	...
	if unscheduled_items:
		notes["unscheduled"] = [
			{"subject": item["subject"], "stream": item["stream"]} for item in unscheduled_items
		]
	if diagnosed:
	...
	if academic_term:
		...
		for a in assignments:
			...
			if room and key not in seen_room:
				seen_room.add(key)
				teaching_rooms.append({"subject": a.subject, "stream": a.stream, "room": room})
...
def generate_timetable(student_groups=None):
	...
	try:
		...
		if student_groups:
			parsed = (
				json.loads(student_groups) if isinstance(student_groups, str) else student_groups
			)
	...
	try:
		...
		return {
			...
			"debug_info": {
				...
				"teacher_count": len({t["teacher"] for t in config["teacher_preferences"]}),
