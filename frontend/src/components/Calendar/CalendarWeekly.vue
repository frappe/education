...
// find the array from array of arrays where the current date is present and return the array in which today's date is present
let currentWeek = props.weeklyDates.findIndex(week => week.find(date => new Date(date).toLocaleDateString().split("T")[0] === new Date().toLocaleDateString().split("T")[0]))
